...
An optional PySide6 GUI for watching this progress lives in the top-level `simnexus_gui/` package (kept outside `simnexus` so PySide6 stays an optional dependency; install with `pip install simnexus[gui]`). Run it standalone with `python -m simnexus_gui [results_dir]`, or embed its `RunProgressWidget`/`StatusView` widgets as a drop-in window in a larger Qt application. It is a pure reader built on `RunWatcher`/`StatusWatcher` and drives `poll()` from a `QTimer`. See `simnexus_gui/README.md`.
