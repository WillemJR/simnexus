...
class d3plot_Open(WorkFlow):
    ...
    def check_if_d3plot_child(cls):
        assert isinstance( cls.parent, d3plot_Open  ), f'd3plot extraction \'{cls.name}\' must be created using D3PlotOperations.add_action'





class d3plot_MetaData(WorkAction):
    """
...
class d3plot_NodalFieldData(WorkAction):
    """
    Arguments:
        state (int): -1 is last
        component (str):
    ...
    def eval( self,  val_dict=None ):
        d3plot_Open.check_if_d3plot_child(self)
        d3p = self.parent.d3plot
        try:
...
class d3plot_MultNodalFieldData(WorkAction):
    """
    Arguments:
        state (int): -1 is last
...
class d3plot_MultElementNodalFieldData(WorkAction):
    """
    Arguments:
        state (int): -1 is last
        required_part_id (int):
...
class d3plot_MultElementFieldData(WorkAction):
    """
    Arguments:
        state (int): -1 is last
        required_part_id (int):
...
class d3plot_NodalValue( d3plot_NodalFieldData):
    """
    Arguments:
        nid (int) :
        state (int) :
...
class d3plot_NodalHistory( d3plot_NodalValue):
    """
    Arguments:
        nid (int) :
        component (str) :
